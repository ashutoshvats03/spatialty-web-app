...
function Page() {
    ...
    return (
        <div>
            <div className="location text-2xl font-extrabold mx-16 mt-5 mb-10">Mumbai-Pune Road</div>
            <div className={`text bg-gray-800 text-center border-r-4 border-b-4 text-[16px] border-white rounded-lg border-2 mx-16 ${displayContent ? "block" : "hidden"}`}>
                    ...
                    {projects && projects.map((project, index) => (
                        <div onClick={() => { dispatch(setProject(project[0])); alert(project[0]), setheader(project[1]); dispatch(setdisplayContent(false)) }} className='border font-bold text-xl border-white' key={index}>
                            <div className={`flex my-5 mx-16 gap-10`}>
                                ...
                                <div className={`text-center flex-[0.5] px-2 py-1 text-sm font-medium ${index !== 0 ? "text-green-700 bg-green-300 rounded-xl" : ""}`}>
                                    {project[3]}
                                </div>
                            </div>
                        </div>
            ...
            <div className={`border-4 border-white mx-16 text-center my-10 ${displayContent ? "hidden" : "block"}`}>
                <div className='font-extrabold text-3xl bg-slate-700 rounded-lg py-5'>
                    {header}
                </div>
                <div className={`bg-slate-700 px-10`}>
                    {projects && projects[2]}
                    <DataComponents piedata={piedata} vehicleData={vehicleData} />
                </div>
            </div>
        </div>
    );
